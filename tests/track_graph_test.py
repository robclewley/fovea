...
cc = fovea.calc_context(sim, 'cannon_traj')
w = cc.workspace
...
max_dist = fovea.make_measure('maxdist',
                              'max(sim.pts["x"])')
max_height = fovea.make_measure('maxheight',
                              'max(sim.pts["y"])')

cc.attach((max_dist, max_height))

tracker(cc, 2, ('angle', 'maxdist', 'ko'),
        clear_on_refresh=False)
tracker(cc, 2, ('angle', 'maxheight', 'ro'),
        clear_on_refresh=False)
